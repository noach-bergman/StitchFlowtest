...
const TasksBoard: React.FC<TasksBoardProps> = ({
  ...
  const canWriteTasks = hasActionAccess(currentUser, 'tasks.write');
  const canDeleteTasks = hasActionAccess(currentUser, 'tasks.delete');
  const canCreateTask = canWriteTasks;
  ...
  const canEditTask = (task: Task): boolean => {
    ...
    if (task.status === 'הושלם' && !canDeleteTasks) return false;
    return true;
  };
  ...
  const openClearCompletedModal = () => {
    if (!canWriteTasks || completedTasksCount === 0) return;
