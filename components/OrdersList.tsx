...
import { Order, Client, OrderStatus, Folder, User } from '../types';
...
const OrdersList: React.FC<OrdersListProps> = ({ orders, clients, folders, setOrders, onDeleteOrder, currentUser, onCreateTaskFromOrder }) => {
  ...
  const canCreateTask = hasActionAccess(currentUser, 'tasks.write') && !!onCreateTaskFromOrder;
  ...
  return (
    <div className="space-y-5 md:space-y-6 text-right pb-10">
      ...
      <div className="bg-white p-3 md:p-2 rounded-3xl md:rounded-2xl border border-rose-100 shadow-sm overflow-x-auto no-scrollbar flex gap-4" dir="rtl">
        {['פעיל', 'הכל', 'חדש', 'מדידות', 'בתפירה', 'מוכן'].map((filter) => (
          <button
            ...
            className={`whitespace-nowrap px-8 md:px-4 py-5 md:py-3 rounded-2xl text-sm md:text-xs font-black transition-all active:scale-95 ${
              activeFilter === filter ? 'bg-rose-600 text-white shadow-xl shadow-rose-100' : 'text-[#7A7A7A] hover:bg-[#fff3f9]'
            }`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredOrders.map(order => {
          ...
          return (
            <div key={order.id} className={`bg-white rounded-[2.5rem] p-8 shadow-sm border border-rose-100 relative group transition-all active:scale-[0.98] ${delivered ? 'opacity-60 grayscale' : ''}`}>
              <div className={`absolute top-0 right-0 w-2.5 h-full rounded-r-full ${delivered ? 'bg-emerald-400' : STATUS_COLORS[order.status].split(' ')[0]}`} />
                ...
                <div className="min-w-0 text-right flex-1 pr-4">
                  ...
                  <h3 className="text-xl md:text-base font-black text-[#2B2B2B] mt-2 truncate leading-tight">{order.itemType}</h3>
                  <p className="text-sm text-[#7A7A7A] font-bold truncate mt-1">{order.clientName}</p>
                </div>
              ...
              <div className="bg-[#fff1f8] rounded-3xl p-5 mb-6 flex items-center justify-between text-right border border-rose-100">
                <ChevronRight className="w-5 h-5 text-gray-300 rotate-180" />
                <div className="flex items-center gap-2 text-xs text-[#7A7A7A] font-bold">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                {!delivered ? (
                  <select 
                    ...
                    className="text-sm font-black bg-rose-50 text-rose-600 px-5 py-3 rounded-2xl outline-none cursor-pointer text-right border border-rose-100 shadow-sm active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
