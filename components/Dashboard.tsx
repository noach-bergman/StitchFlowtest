...
import { Client, Order, Folder, PagePermissionId, User } from '../types';
import { TrendingUp, Users, Scissors, Clock, FolderOpen, Package, Wallet, Cloud, ShieldCheck, ListTodo, Eye } from 'lucide-react';
import { hasPageAccess } from '../services/permissions';

interface DashboardProps {
  clients: Client[];
  folders: Folder[];
  orders: Order[];
  onNavigate: (tab: PagePermissionId) => void;
  currentUser: User;
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const canViewFolders = hasPageAccess(currentUser, 'folders');
  const canViewPayments = hasPageAccess(currentUser, 'payments');
  const canViewDataManagement = hasPageAccess(currentUser, 'data-mgmt');
  const canViewUsers = hasPageAccess(currentUser, 'users');
    ...
    >
      ...
      <div className="rounded-[1.85rem] md:rounded-[2rem] p-6 md:p-8 text-right border bg-white/95" style={{ borderColor: '#e5488676', boxShadow: '0 12px 25px rgba(229,72,134,0.16)' }}>
         ...
         <div className="grid grid-cols-3 gap-4">
            {hasPageAccess(currentUser, 'tasks') && <NavGridButton label="משימות" icon={<ListTodo />} onClick={() => onNavigate('tasks')} />}
            {hasPageAccess(currentUser, 'clients') && <NavGridButton label="לקוחות" icon={<Users />} onClick={() => onNavigate('clients')} />}
            {hasPageAccess(currentUser, 'folders') && <NavGridButton label="תיקים" icon={<FolderOpen />} onClick={() => onNavigate('folders')} />}
            {hasPageAccess(currentUser, 'orders') && <NavGridButton label="הזמנות" icon={<Scissors />} onClick={() => onNavigate('orders')} />}
            {canViewPayments && <NavGridButton label="תשלומים" icon={<Wallet />} onClick={() => onNavigate('payments')} />}
            {hasPageAccess(currentUser, 'inventory') && <NavGridButton label="מלאי" icon={<Package />} onClick={() => onNavigate('inventory')} />}
            {canViewDataManagement && <NavGridButton label="ענן" icon={<Cloud />} onClick={() => onNavigate('data-mgmt')} />}
            {canViewUsers && <NavGridButton label="צוות" icon={<ShieldCheck />} onClick={() => onNavigate('users')} />}
         </div>
      </div>
    </div>
  );
