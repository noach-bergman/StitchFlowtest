...
import { Client, Folder, Order, OrderStatus, PaymentStatus, User } from '../types';
...
import { generateProfessionalReceipt, ReceiptGenerationError } from '../services/openaiReceipt';
...
const ClientFolders: React.FC<ClientFoldersProps> = ({ 
  ...
  onDeleteFolder, onDeleteOrder, currentUser, initialFolderId, highlightedOrderId, onClearInitialFolder,
  ...
  const [receiptErrorRequestId, setReceiptErrorRequestId] = useState<string | null>(null);
  ...
  const canWriteFolders = hasActionAccess(currentUser, 'folders.write');
  const canDeleteFolders = hasActionAccess(currentUser, 'folders.delete');
  const canArchiveFolders = hasActionAccess(currentUser, 'folders.archive');
  const canWriteOrders = hasActionAccess(currentUser, 'orders.write');
  const canDeleteOrders = hasActionAccess(currentUser, 'orders.delete');
  const canWritePayments = hasActionAccess(currentUser, 'payments.write');
  const canWriteClients = hasActionAccess(currentUser, 'clients.write');
  const canCreateTask = hasActionAccess(currentUser, 'tasks.write');
  ...
  const handleGenerateReceipt = async () => {
    ...
    try {
      ...
      console.error('Receipt generation UI flow failed:', error);
      const requestId = error instanceof ReceiptGenerationError ? error.requestId : undefined;
      const message = error instanceof Error ? error.message : 'Receipt generation failed';
      setReceiptErrorMessage(message);
      setReceiptErrorRequestId(requestId || null);
      showUiAlert(
        requestId
          ? `יצירת הקבלה נכשלה. מזהה תקלה: ${requestId}`
          : 'יצירת הקבלה נכשלה. בדקי את לוגי השרת לפרטים.',
      );
    } finally {
  ...
  const handleAddFolder = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!canWriteFolders) {
      showUiAlert('אין לך הרשאה ליצור תיקים');
      return;
    }
  ...
  const handleAddPayment = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!selectedFolder || !canWritePayments) {
      showUiAlert('אין לך הרשאה לעדכון תשלומים');
      return;
    }
  ...
  const handleAddOrder = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!canWriteOrders) {
      showUiAlert('אין לך הרשאה לעדכן הזמנות');
      return;
    }
  ...
  const handleQuickAddClient = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!canWriteClients) {
      showUiAlert('אין לך הרשאה להוסיף לקוחות');
      return;
    }
    const formData = new FormData(e.currentTarget);
  ...
  const toggleArchive = (folderId: string) => {
    if (!canArchiveFolders) {
      showUiAlert('אין לך הרשאה לארכוב תיקים');
      return;
    }
  ...
  return (
    <div className="space-y-6 text-right pb-32">
           ...
           <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-rose-100 relative overflow-hidden">
             ...
             <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
                <button 
                  ...
                  className="w-full md:w-auto bg-gradient-to-r from-rose-600 to-pink-600 text-white px-10 py-5 rounded-[2rem] font-black text-lg shadow-2xl shadow-rose-200 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
           ...
           <div className="bg-white rounded-[3rem] p-10 md:p-12 shadow-2xl border border-rose-100 relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-12">
                 ...
                 <button
                    onClick={() => {
                      if (!canWriteFolders) {
                        showUiAlert('אין לך הרשאה לעדכון תיקים');
                        return;
                      }
                      setFolders(folders.map(f => f.id === selectedFolderId ? { ...f, isDelivered: !f.isDelivered } : f));
                    }}
                    disabled={!canWriteFolders}
                    className={`flex flex-col items-center gap-3 py-6 rounded-[2rem] border transition-all disabled:opacity-60 disabled:cursor-not-allowed ${selectedFolder?.isDelivered ? 'bg-emerald-500 text-white border-transparent shadow-lg' : 'bg-[#fff1f8] border-rose-200 text-[#7A7A7A]'}`}
                 >
                 ...
                 <button
                    ...
                    disabled={!canArchiveFolders}
                    className={`flex flex-col items-center gap-3 py-6 rounded-[2rem] border ${
                      canMoveSelectedFolderToArchive && canArchiveFolders
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => {
                    if (!canWriteOrders) return;
                    setEditingOrder(null);
                    setTempDisplayId(generateNewDisplayId());
                    setIsOrderModalOpen(true);
                  }}
                  disabled={!canWriteOrders}
                  className="bg-rose-600 text-white px-8 py-4 rounded-full font-black text-sm active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  הוסף פריט +
                </button>
              </div>
                ...
                >
                   <div className="absolute top-6 left-6 flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setTempDisplayId(order.displayId);
                          setIsOrderModalOpen(true);
                        }}
                        disabled={!canWriteOrders}
                        className="p-3 bg-indigo-50 text-indigo-500 rounded-xl active:scale-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        <Edit2 size={16} />
                      </button>
                      {canDeleteOrders && <button onClick={() => setOrderToDelete(order)} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90 transition-all"><Trash2 size={16} /></button>}
                   </div>
                   ...
                   <div className="mt-8 pt-6 border-t border-gray-50 flex justify-between items-center">
                      <select 
                        ...
                        className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest outline-none cursor-pointer border shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed ${STATUS_COLORS[order.status]}`}
                      >
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-[#fff1f8]">
                {isGeneratingReceipt ? (
                   ...
                   <div className="text-center py-10">
                      ...
                      {receiptErrorMessage && (
                        <p className="mt-3 text-sm text-[#7A7A7A] font-medium">{receiptErrorMessage}</p>
                      )}
                      {receiptErrorRequestId && (
                        <p className="mt-2 text-xs font-mono text-rose-700">Request ID: {receiptErrorRequestId}</p>
                      )}
      ...
      {folderToArchive && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-[#5f1f4b66] backdrop-blur-[3px]">
           <div className="relative bg-gradient-to-b from-white to-[#fff4f8] border border-[#e5488675] rounded-[2.2rem] p-8 md:p-10 max-w-md w-full text-center shadow-[0_20px_45px_rgba(229,72,134,0.24)] overflow-hidden">
              <div className="absolute -top-8 -left-8 w-24 h-24 rounded-full bg-indigo-200/25 blur-2xl" />
              <div className="absolute -bottom-10 right-6 w-28 h-28 rounded-full bg-rose-200/30 blur-2xl" />

              <div className="relative w-20 h-20 bg-[#efe9ff] border border-[#9b7bd84d] rounded-[1.5rem] flex items-center justify-center text-[#7b5cb7] mx-auto mb-6 shadow-sm">
                <Archive size={36} />
              </div>
              <h3 className="text-2xl font-black text-[#2B2B2B] mb-2 font-heebo">העברה לארכיון</h3>
              <p className="text-sm text-[#7A7A7A] mb-8 leading-7 font-bold">
                האם להעביר את <b className="text-[#2B2B2B]">{folderToArchive.name}</b> לארכיון?
              </p>
              <div className="flex gap-3">
                 <button
                   onClick={() => setFolderToArchive(null)}
                   className="flex-1 py-3.5 rounded-2xl border border-[#e5488652] bg-white text-[#7A7A7A] font-black active:scale-95 hover:bg-[#fff5fa] transition-all duration-200"
                 >
                   ביטול
                 </button>
                 <button
                   onClick={() => toggleArchive(folderToArchive.id)}
                   disabled={!canArchiveFolders}
                   className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-[#E54886] to-[#F26AA3] text-white font-black shadow-[0_10px_24px_rgba(229,72,134,0.28)] active:scale-95 transition-all duration-200"
                 >
                   העבר
                 </button>
              </div>
      ...
      {folderToDelete && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-[#5f1f4b66] backdrop-blur-[3px]">
           <div className="relative bg-gradient-to-b from-white to-[#fff4f8] border border-[#e5488675] rounded-[2.2rem] p-8 md:p-10 max-w-md w-full text-center shadow-[0_20px_45px_rgba(229,72,134,0.24)] overflow-hidden">
              <div className="absolute -top-8 -left-8 w-24 h-24 rounded-full bg-rose-200/40 blur-2xl" />
              <div className="absolute -bottom-10 right-6 w-28 h-28 rounded-full bg-rose-200/40 blur-2xl" />

              <div className="relative w-20 h-20 bg-[#ffe8f3] border border-[#e548865f] rounded-[1.5rem] flex items-center justify-center text-[#E54886] mx-auto mb-6 shadow-sm">
                <ShieldAlert size={36} />
              </div>
              <h3 className="text-2xl font-black text-[#2B2B2B] mb-2 font-heebo">מחיקה סופית</h3>
              <p className="text-sm text-[#7A7A7A] mb-8 leading-7 font-bold">
                זהירות! כל המידע על <b className="text-[#2B2B2B]">{folderToDelete.name}</b> יימחק לצמיתות.
              </p>
              <div className="flex gap-3">
                 <button
                 ...
                 <button
                   ...
                   className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-[#E54886] to-[#F26AA3] text-white font-black shadow-[0_10px_24px_rgba(229,72,134,0.28)] active:scale-95 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
                 >
                   מחק הכל
                 </button>
              </div>
      ...
      {orderToDelete && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-[#5f1f4b66] backdrop-blur-[3px]">
           <div className="relative bg-gradient-to-b from-white to-[#fff4f8] border border-[#e5488675] rounded-[2.2rem] p-8 md:p-10 max-w-md w-full text-center shadow-[0_20px_45px_rgba(229,72,134,0.24)] overflow-hidden">
              <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-rose-200/35 blur-2xl" />
              <div className="absolute -bottom-10 left-6 w-28 h-28 rounded-full bg-rose-200/35 blur-2xl" />

              <div className="relative w-20 h-20 bg-[#ffe8f3] border border-[#e548865f] rounded-[1.5rem] flex items-center justify-center text-[#E54886] mx-auto mb-6 shadow-sm">
                <Trash2 size={36} />
              </div>
              <h3 className="text-2xl font-black text-[#2B2B2B] mb-2 font-heebo">מחיקת פריט</h3>
              <p className="text-sm text-[#7A7A7A] mb-8 leading-7 font-bold">
                האם למחוק את <b className="text-[#2B2B2B]">{orderToDelete.itemType}</b>?
              </p>
              <div className="flex gap-3">
                 <button
      ...
      {pendingPriceDateUpdate && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-[#5f1f4b66] backdrop-blur-[3px]">
           <div className="relative bg-gradient-to-b from-white to-[#fff4f8] border border-[#e5488675] rounded-[2.2rem] p-8 md:p-10 max-w-md w-full text-center shadow-[0_20px_45px_rgba(229,72,134,0.24)] overflow-hidden">
              ...
              <div className="absolute -bottom-10 left-6 w-28 h-28 rounded-full bg-rose-200/30 blur-2xl" />

              <div className="relative w-20 h-20 bg-[#fff4e6] border border-[#d9a6495a] rounded-[1.5rem] flex items-center justify-center text-[#c98d1c] mx-auto mb-6 shadow-sm">
                <AlertTriangle size={36} />
              </div>
              <h3 className="text-2xl font-black text-[#2B2B2B] mb-2 font-heebo">עדכון תאריך הכנסה</h3>
              <p className="text-sm text-[#7A7A7A] mb-8 leading-7 font-bold">
                שינית את המחיר של <b className="text-[#2B2B2B]">{pendingPriceDateUpdate.order.itemType}</b>.
                האם לעדכן את תאריך ההכנסה להיום?
              </p>
              <div className="flex gap-3">
                 <button
                   onClick={() => resolvePendingPriceDateUpdate(false)}
                   className="flex-1 py-3.5 rounded-2xl border border-[#e5488652] bg-white text-[#7A7A7A] font-black active:scale-95 hover:bg-[#fff5fa] transition-all duration-200"
                 >
                   השאר תאריך
                 </button>
                 <button
                   onClick={() => resolvePendingPriceDateUpdate(true)}
                   className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-[#E54886] to-[#F26AA3] text-white font-black shadow-[0_10px_24px_rgba(229,72,134,0.28)] active:scale-95 transition-all duration-200"
                 >
                   עדכן להיום
                 </button>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full text-center shadow-2xl">
            ...
            <form onSubmit={paymentModalMode === 'add' ? handleAddPayment : handleSetPaidAmount} className="space-y-5 text-right">
              ...
              <div className="flex gap-4">
                ...
                <button
                  type="submit"
                  disabled={!canWritePayments || (paymentModalMode === 'add' && selectedAddPaymentLimit <= 0)}
           ...
           <div className="bg-white rounded-[3rem] w-full max-w-lg shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-hidden">
              ...
              <div className="p-10 space-y-8 text-right overflow-y-auto max-h-[70vh]">
                 ...
                 <form onSubmit={handleAddFolder} className="space-y-8">
                    ...
                    <button type="submit" disabled={!selectedClientId || !canWriteFolders} className="w-full bg-rose-600 text-white font-black text-xl py-6 rounded-3xl shadow-2xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">צור תיק עבודה</button>
                 </form>
              </div>
           </div>
        </div>
      ...
      {isOrderModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-[#5f1f4b66] backdrop-blur-[3px] no-print">
           <div className="bg-white border border-[#e5488670] rounded-[3rem] w-full max-w-lg shadow-[0_20px_48px_rgba(229,72,134,0.24)] animate-in slide-in-from-bottom duration-300 overflow-hidden">
              <div className="relative p-8 bg-gradient-to-r from-[#ffe6f2] via-[#fff1f8] to-[#ffe6f2] border-b border-[#e5488645] text-[#2B2B2B] flex justify-between items-center">
                 <div className="absolute -top-10 -left-6 w-24 h-24 rounded-full bg-rose-300/20 blur-2xl" />
                 <div className="absolute -bottom-10 right-6 w-24 h-24 rounded-full bg-rose-300/20 blur-2xl" />
                 <div className="relative text-right">
                   <h3 className="text-2xl font-black font-heebo">{editingOrder ? 'עריכת תיקון' : 'פתיחת תיקון'}</h3>
                   <p className="text-xs font-bold text-[#a95b83] mt-1">הגדירי פרטי תיקון ושמרי לתיק הלקוחה</p>
                 </div>
                 <button
                   onClick={() => setIsOrderModalOpen(false)}
                   className="relative p-2 text-[#a95b83] hover:text-[#E54886] hover:bg-white/70 rounded-full border border-[#e5488640] transition-all"
                 >
                   <X size={32} />
                 </button>
              </div>
              <form onSubmit={handleAddOrder} className="p-10 space-y-8 text-right">
                 <div className="bg-gradient-to-r from-[#E54886] to-[#F26AA3] p-6 rounded-[2rem] flex items-center justify-between shadow-[0_10px_24px_rgba(229,72,134,0.25)] border border-[#ffffff55]">
                    <button type="button" onClick={() => setTempDisplayId(generateNewDisplayId())} className="p-3 bg-white/25 hover:bg-white/40 text-white rounded-xl transition-all border border-white/35">
                       <RefreshCw size={20} />
                    </button>
                    <div className="text-right">
                       <label className="text-[9px] font-black text-white/80 uppercase tracking-widest block mb-1">Item ID</label>
                       <div className="text-4xl font-black text-white font-heebo tracking-tighter">#{tempDisplayId}</div>
                    </div>
                    <div className="w-12 h-12 bg-white/25 rounded-2xl flex items-center justify-center text-white border border-white/35">
                       <Hash size={24} />
                    </div>
                 </div>
                 <div className="space-y-2">
                 ...
                 <button type="submit" disabled={!canWriteOrders} className="w-full bg-[#6f2f54] text-white font-black text-xl py-6 rounded-3xl shadow-2xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">שמור פריט</button>
              </form>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md no-print">
           <div className="bg-white rounded-[3rem] w-full max-w-sm shadow-2xl animate-in zoom-in duration-300 overflow-hidden">
              ...
              <form onSubmit={handleQuickAddClient} className="p-8 space-y-6 text-right">
                 ...
                 <button type="submit" disabled={!canWriteClients} className="w-full bg-[#6f2f54] text-white font-black py-5 rounded-3xl shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
