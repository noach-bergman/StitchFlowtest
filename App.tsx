...
import { Client, Order, Fabric, Folder, PagePermissionId, Task, User } from './types';
import { dataService } from './services/dataService';
import { showUiAlert } from './services/uiAlert';
import { hasActionAccess, hasPageAccess, normalizeUserPermissions } from './services/permissions';
import { isSameUserAccessState, resolveSyncedCurrentUser } from './services/sessionUserSync';
import {
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<PagePermissionId>('dashboard');
  ...
  useEffect(() => {
    ...
    try {
      ...
      const normalizedUser = normalizeUserPermissions(parsedUser);
      setCurrentUser(normalizedUser);
      localStorage.setItem('stitchflow_user', JSON.stringify(normalizedUser));
    } catch (error) {
      localStorage.removeItem('stitchflow_user');
    }
  }, []);

  const syncCurrentUserFromUsers = (normalizedUsers: User[]) => {
    setCurrentUser((prevUser) => {
      const syncedUser = resolveSyncedCurrentUser(prevUser, normalizedUsers);
      if (isSameUserAccessState(prevUser, syncedUser)) {
        return prevUser;
      }
      if (syncedUser) {
        localStorage.setItem('stitchflow_user', JSON.stringify(syncedUser));
      }
      return syncedUser;
    });
  };
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const normalizedUsers = (u || []).map((user) => normalizeUserPermissions(user));
  ...
  const handleLogin = (user: User) => {
    const normalizedUser = normalizeUserPermissions(user);
    setCurrentUser(normalizedUser);
    localStorage.setItem('stitchflow_user', JSON.stringify(normalizedUser));
  };
  ...
  const handleDeleteClient = async (clientId: string) => {
    if (!hasActionAccess(currentUser, 'clients.delete')) {
  ...
  const handleDeleteFolder = async (folderId: string) => {
    if (!hasActionAccess(currentUser, 'folders.delete')) {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    if (!hasActionAccess(currentUser, 'orders.delete')) {
  ...
  const navigateToFolder = (folderId: string, orderId?: string) => {
    if (!hasPageAccess(currentUser, 'folders')) {
      showUiAlert('אין לך הרשאה לגשת למסך תיקי לקוחות');
      return;
    }
  ...
  const openTaskFromOrder = (order: Order) => {
    if (!hasPageAccess(currentUser, 'tasks')) {
      showUiAlert('אין לך הרשאה לגשת למסך משימות');
      return;
    }
    setTaskPrefill({
      kind: 'order',
  ...
  const handleNavigate = (tabId: PagePermissionId) => {
    if (!hasPageAccess(currentUser, tabId)) {
      showUiAlert('אין לך הרשאה לגשת לדף זה');
      if (currentUser) {
        const fallbackTab = NAV_ITEMS.find((item) => hasPageAccess(currentUser, item.id))?.id || 'dashboard';
  ...
  const handleUserUpdated = (updatedUser: User) => {
    const normalizedUser = normalizeUserPermissions(updatedUser);
    setUsers((prevUsers) => {
      const exists = prevUsers.some((user) => user.id === normalizedUser.id);
      if (!exists) return [normalizedUser, ...prevUsers];
      return prevUsers.map((user) => (user.id === normalizedUser.id ? normalizedUser : user));
    });

    setCurrentUser((prevUser) => {
      if (!prevUser) return prevUser;
      if (prevUser.id !== normalizedUser.id && prevUser.username !== normalizedUser.username) {
        return prevUser;
      }
      localStorage.setItem('stitchflow_user', JSON.stringify(normalizedUser));
  ...
  const handleUsersReload = async () => {
    try {
      ...
      const normalizedUsers = (fetchedUsers || []).map((user) => normalizeUserPermissions(user));
      setUsers(normalizedUsers);
      syncCurrentUserFromUsers(normalizedUsers);
      setLastSync(new Date());
    } catch (error) {
      showUiAlert('שגיאה בטעינת משתמשים');
    }
  };

  useEffect(() => {
    if (!currentUser) return;
    if (hasPageAccess(currentUser, activeTab)) return;
  ...
  const canAccessIncome = hasPageAccess(currentUser, 'income');
  const isWeeklyRevenueVisible = canAccessIncome && isIncomeUnlocked();
  const requestWeeklyRevenueReveal = () => {
    if (!canAccessIncome) {
      showUiAlert('אין לך הרשאה לחשוף נתוני הכנסות');
      return;
    }
    openIncomeGate('dashboard_weekly');
  };

  const renderContent = () => {
    if (!hasPageAccess(currentUser, activeTab)) {
      return (
        <Dashboard
          clients={clients}
          folders={folders}
          ...
          onRequestWeeklyRevenueReveal={requestWeeklyRevenueReveal}
        />
      );
    }
    ...
    switch (activeTab) {
      ...
      case 'payments':
        if (!hasPageAccess(currentUser, 'payments')) {
          return (
            <Dashboard
              clients={clients}
              folders={folders}
      ...
      case 'income':
        if (!hasPageAccess(currentUser, 'income') || !isIncomeUnlocked()) {
          return (
            <Dashboard
              clients={clients}
              folders={folders}
      ...
      case 'inventory': return <Inventory inventory={inventory} setInventory={handleSaveInventory} currentUser={currentUser} />;
      ...
      case 'users':
        return (
          <UserManagement
            ...
            onUsersReload={handleUsersReload}
          />
        );
      default:
        return (
  ...
  const visibleNavItems = NAV_ITEMS.filter((item) => hasPageAccess(currentUser, item.id));
           ...
           <div className="bg-[#fff0f7] rounded-xl p-3 border border-rose-200 flex items-center gap-3">
              ...
              <div className="min-w-0">
                 ...
                 <p className="text-[9px] text-rose-500 font-black uppercase">
                  {currentUser.role === 'super_admin' ? 'מנהל על' : currentUser.role === 'admin' ? 'מנהל' : currentUser.role === 'viewer' ? 'צפייה' : 'צוות'}
                 </p>
              </div>
           </div>
        </div>
